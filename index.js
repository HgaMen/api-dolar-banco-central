...
app.get('/', cors(), async (req, res) => {
  ...
  try {
    ...
    // const dataBitcoin = await axios.get("https://mx.investing.com/crypto/bitcoin/btc-usd")
    ...
    // if (dataBC.status === 'rejected') throw error({ msg: dataBC.reason });
    ...
    const objetoDolar = {
      text: 'dolar',
      valor: $(selectorDolar).html().split('/')[0].trim() ?? 'sin datos',
      fecha: new Date().toLocaleDateString(),
    };

    const objetoUF = {
      text: 'UF',
      valor: $(selectorUF).text() ?? 'sin datos', // Nullish coalescing operator / Operador coalescente si el nulo operando izquierdo es un valor falso , no solo nullo undefined
      fecha: new Date().toLocaleDateString(),
    };

    const objetoEuro = {
      text: 'Euro',
      valor: $(selectorEuro).text() ?? 'sin datos', // Nullish coalescing operator / Operador coalescente si el nulo operando izquierdo es un valor falso , no solo nullo undefined
      fecha: new Date().toLocaleDateString(),
    };
    ...
    // console.log($(selectorBitcoin).text());

    const objetoBitcoin = {
      text: 'Bitcoin',
      valor: $(selectorBitcoin).text() ?? 'sin datos', // Nullish coalescing operator / Operador coalescente si el nulo operando izquierdo es un valor falso , no solo nullo undefined
      fecha: new Date().toLocaleDateString(),
    };
